...
- **NAS(smb://163.180.10.191, 공유 `cowork/EV`)**: 대용량 데이터 산출물의 기본 저장 위치.
  ...
  - `input/processed/` — raw로부터 가공한 분석 입력(도로망_그래프, yearly_snapshots,
    D1 집계 csv 등)
  - `output/` — G2SFCA 최종 결과(`g2sfca`, `g2sfca_s2`, `g2sfca_s2_park10`)
  - `reference/` — 선행논문 PDF
...
- **`archive/`** — 더 이상 쓰이지 않는 코드와 산출물(연구주제 피벗으로 보류된 7개 도시+경기도
  ...
  삭제 대신 이력 보존 목적으로 이동, 각 항목의 대체 이유는 `archive/README.md` 참고
...
- 공급(S)은 3가지 정의로 전체 80개 조합(4개년×2요일유형×[오전/낮/밤 3시나리오 + 심야 1시나리오])을
  ...
  - **S1**: 충전소 단순 대수(무가중) — `/mnt/cowork/EV/output/g2sfca/`
  - **S2(채택)**: 급속:완속 = 2.4:1, 프로젝트 자체 10분 가용률 루프 실측(세션 빈도 기반) —
    `/mnt/cowork/EV/output/g2sfca_s2/`
  - **s2park**: 급속:완속 = 10:1, Park et al.(2022, 강남·서초·송파) 문헌값 — 강건성 비교용,
    `/mnt/cowork/EV/output/g2sfca_s2_park10/`
...
- 거리조락함수(distance decay)도 `--decay {binary,gaussian}`으로 파라미터화(2026-08-13) —
  기본값 binary(t0=15분 이내 가중치 1)는 그동안 검토 없이 쓰던 단순화, gaussian(Luo&Qi 2009
  E2SFCA 공식)은 교수님 논문(Park et al. 2022) 원 코드가 실제로 쓰는 방식이라 강건성 비교용
  추가. S2 기준 두 방식 다 2021→2024 증가율 4.67배로 동일(강건함)하지만, 공간 패턴 상관관계는
  0.885로 공급가중치 비교(0.996)보다 뚜렷이 낮음 — decay 함수 모양이 "어디가 접근성 좋은가"의
  공간 분포 자체에 더 큰 영향을 줌. Notion 2026-08-13 토글에 상세 기록
