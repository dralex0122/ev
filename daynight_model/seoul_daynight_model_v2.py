"""
서울시 주/야간 인구 회귀분석 v2: 기존 (등록인구+주택수+종사자수)에
건축물대장 총괄표제부의 연면적(주거용/비주거용 구분)을 추가해 재적합.

입력:
- ~/ev-charger-accessibility/gridpop_7cities_verified.csv (100m 인구, 서울만 사용)
- ~/ev-charger-accessibility/grid_stats/housing/*.csv (100m 총주택수, to_ho_001)
- ~/ev-charger-accessibility/grid_stats/employ/*.csv (100m 총종사자수, to_em_020)
- ~/ev-charger-accessibility/grid_stats/seoul_250m_생활인구_평일평균프로파일*.csv (250m 주/야간 실측)
- ~/ev-charger-accessibility/building_register_seoul_geocoded.csv (지오코딩된 건축물대장, 위경도)
"""
import csv
import glob
import os
...
# 주거용으로 볼 주용도코드명 키워드 (건축물대장 표준 용도 분류 기준)
...
def latlon_to_cell250_id(lat, lon, default_prefix="다사"):
    x, y = WGS84_TO_5179.transform(lon, lat)
    # 서울은 대부분 다사 타일 내부이므로 다사 기준으로 계산하되,
    # 타일 원점 밖으로 벗어나면 인접 타일로 보정
...
def main():
    # 1) 서울 100m 인구
    pop_by_grid = {}
    with open(os.path.join(BASE, "gridpop_7cities_verified.csv"), encoding="utf-8-sig") as f:
        r = csv.DictReader(f)
        for row in r:
            if row["city"] == "서울특별시":
    ...
    # 5) 건축물대장(지오코딩됨) -> 250m 집계, 주거용/비주거용 연면적 분리
    bldg_fp = os.path.join(BASE, "building_register_seoul_geocoded.csv")
    matched_bldg, total_bldg = 0, 0
    ...
    print("\n=== [기존] night ~ pop+housing+employ ===")
    r2, s, _ = fit_ols(np.column_stack([pop, housing, employ]), night, ["pop", "housing", "employ"])
    print(f"R2={r2:.4f}  {s}")

    print("\n=== [기존] day ~ pop+housing+employ ===")
    r2, s, _ = fit_ols(np.column_stack([pop, housing, employ]), day, ["pop", "housing", "employ"])
    print(f"R2={r2:.4f}  {s}")

    print("\n=== [v2] night ~ pop+housing+employ+floor_res+floor_nonres ===")
    r2, s, night_pred = fit_ols(np.column_stack([pop, housing, employ, floor_res, floor_nonres]), night,
                                 ["pop", "housing", "employ", "floor_res", "floor_nonres"])
    print(f"R2={r2:.4f}  {s}")

    print("\n=== [v2] day ~ pop+housing+employ+floor_res+floor_nonres ===")
    r2, s, day_pred = fit_ols(np.column_stack([pop, housing, employ, floor_res, floor_nonres]), day,
                               ["pop", "housing", "employ", "floor_res", "floor_nonres"])
    print(f"R2={r2:.4f}  {s}")

    out_fp = os.path.join(BASE, "seoul_daynight_model_v2_joined.csv")
