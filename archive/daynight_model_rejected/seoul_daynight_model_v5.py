...
def main():
    ...
    # ---- v5: 로그 변환(log1p) 후 재적합 ----
    log_pop = np.log1p(pop)
    log_employ = np.log1p(employ)
    log_floor_res = np.log1p(floor_res)
    log_floor_nonres = np.log1p(floor_nonres)
    log_night = np.log1p(night)
    log_day = np.log1p(day)

    print("\n=== [v5, 로그변환] log(night) ~ log(pop)+log(employ)+log(floor_res)+log(floor_nonres) ===")
    r2_log, s, log_night_pred = fit_ols(
        np.column_stack([log_pop, log_employ, log_floor_res, log_floor_nonres]), log_night,
        ["log_pop", "log_employ", "log_floor_res", "log_floor_nonres"])
    print(f"R2(log space)={r2_log:.4f}  {s}")
    night_pred_v5 = np.expm1(log_night_pred)
    ss_res = np.sum((night - night_pred_v5) ** 2)
    ss_tot = np.sum((night - np.mean(night)) ** 2)
    r2_orig = 1 - ss_res / ss_tot
    print(f"R2(원래 스케일로 역변환 후 비교)={r2_orig:.4f}")

    print("\n=== [v5, 로그변환] log(day) ~ log(pop)+log(employ)+log(floor_res)+log(floor_nonres) ===")
    r2_log, s, log_day_pred = fit_ols(
        np.column_stack([log_pop, log_employ, log_floor_res, log_floor_nonres]), log_day,
        ["log_pop", "log_employ", "log_floor_res", "log_floor_nonres"])
    print(f"R2(log space)={r2_log:.4f}  {s}")
    day_pred_v5 = np.expm1(log_day_pred)
    ss_res = np.sum((day - day_pred_v5) ** 2)
    ss_tot = np.sum((day - np.mean(day)) ** 2)
    r2_orig = 1 - ss_res / ss_tot
    print(f"R2(원래 스케일로 역변환 후 비교)={r2_orig:.4f}")
    ...
    out_fp = os.path.join(BASE, "daynight_model", "seoul_daynight_model_v5_joined.csv")
    with open(out_fp, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        w.writerow(["cell250_id", "pop", "housing", "employ", "floor_res", "floor_nonres",
                    "night_avg", "day_avg", "night_pred_v4", "day_pred_v4",
                    "night_pred_v5_log", "day_pred_v5_log"])
        for i, r in enumerate(rows):
            w.writerow(list(r) + [f"{night_pred[i]:.2f}", f"{day_pred[i]:.2f}",
                                   f"{night_pred_v5[i]:.2f}", f"{day_pred_v5[i]:.2f}"])
    print(f"\n조인 결과 저장: {out_fp}")
