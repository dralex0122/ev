...
for decay_key, decay_label in DECAYS.items():
    NAS = f"/mnt/cowork/EV/output/g2sfca_sfast_simya_{decay_key}"
    ...
    vmin = 0
    bounds_report[f"simya_{decay_key}"] = (vmin, vmax)
    ...
    for year in YEARS:
        ...
        fig.suptitle(f"{year}년 · 심야(00~05시)", fontsize=16, color=INK, fontweight="bold", y=0.97)
        fig.text(0.5, 0.925, f"G2SFCA 접근성 ({decay_label}) · 급속+아파트제외+운영시간반영 · freeflow · 평균 {mean_val*1000:.3f}‰",
                  ha="center", fontsize=10.5, color=MUTED)

        sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=vmin, vmax=vmax))
        sm.set_array([])
        cbar = fig.colorbar(sm, ax=ax, orientation="horizontal", fraction=0.035, pad=0.02, shrink=0.7)
        cbar.set_label("접근성 점수 (높을수록 좋음, 4개 연도 공통 스케일)", color=SECONDARY_INK, fontsize=9)
        cbar.ax.xaxis.set_tick_params(color=MUTED, labelcolor=MUTED)
        cbar.outline.set_edgecolor(MUTED)

        out_fp = f"{OUT_DIR}/simya_{decay_key}_{year}.png"
