"""
2026-08-27 랩미팅 지시: Gaussian 2SFCA vs Gravity Model vs Cumulative Opportunity
3개 모형 비교용 Gi* 핫스팟/콜드스팟 분석.

범위: 2021~2024년 평일 낮(week_낮_normal) 단일 시나리오, 3개 모형 x 4개년 = 12개 조합.
...
MODELS = {
    ...
    "CumOpp": lambda year: f"{NAS}/output/cumulative_opportunity/cumopp_score_{year}_week_낮_normal.csv",
}

OUT_FP = f"{NAS}/output/three_model_hotspot_k30.csv"
...
def main():
    ...
    print(f"\n>> 저장 완료: {OUT_FP} ({len(df):,}행 = 12개 조합 x {len(gdf):,}집계구)")
