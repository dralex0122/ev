...
EXCLUDED_STATION_IDS = {
    "KP003057",
    "LU000359",
    # 원본 API에 zcode=26(부산광역시)으로 등록돼 있지만 addr은 다른 도시를 가리키는
    # 케이스. 부산에도 강서구/서구/남구/북구가 실제로 존재해서 구 이름이 우연히
    # 겹치는 바람에, 걸러내지 않으면 서울/대전/울산 데이터가 부산 구/군 집계에
    # 조용히 섞여 들어감. addr이 가리키는 진짜 도시(zcode=11/30/31)로 조회할 땐
    # 애초에 API가 이 statId를 내려주지 않으므로, 그 도시 집계에는 포함되지 않음
    "EV017663",  # 서울강서 강변힐스테이트아파트 1 -> addr: 서울특별시 강서구
    "EV018042",  # 서울강서 영풍콜택시 3 -> addr: 서울특별시 강서구
    "KP008097",  # 서대전지사(공용) -> addr: 대전광역시 서구
    "KP008107",  # 동울산지사(공용) -> addr: 울산광역시 북구
    "KP008111",  # 울산지사(공용) -> addr: 울산광역시 남구
}
